...
def solve_iteration(x_values, density_values, momentum_values, energy_values, N, dt, dx, gamma, simulation_time, scheme_name):
	...
	for iteration in range(num_iterations):
		...
		if scheme_name == "Rusanov":	
			...
			Lambda_R = np.array(velocity_values[1:N+1])  
			Lambda_L = np.array(velocity_values[1:N+1])
	...
	plt.plot(x_values, density_values[1:-1], label = scheme_name + "solution")
	plt.title("Sod Shock - {}: Density Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Density")
	plt.savefig("{}density_N{}_{}.png".format(saved_graphs_directory, N-1, scheme_name))
	plt.pause(1)
	plt.close()

	plt.plot(x_values, velocity_values[1:-1], label = scheme_name + "solution")
	plt.title("Sod Shock - {}: Velocity Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Velocity")
	...
	plt.plot(x_values, pressure_values[1:-1], label = scheme_name + "solution")
	plt.title("Sod Shock- {}: Pressure Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Pressure")
	plt.savefig("{}pressure_N{}_{}.png".format(saved_graphs_directory, N-1, scheme_name))
	plt.pause(1)
	plt.close()
...
saved_graphs_directory = "Q1_graphs/"
...
for N in N_values:
	...
	density_values = initialize_grid(N, "density", x_values)
	velocity_values = initialize_grid(N, "velocity", x_values)
	pressure_values = initialize_grid(N, "pressure", x_values)
