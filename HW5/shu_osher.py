...
def initialize_grid(N, var, x_values):
	...
	for i in range(1,N+1):
		...
		if x<-4:
			...
			if var == "density":	
				grid_values[i,0] = 1+0.2*np.sin(5*x)
			else:
...
def solve_iteration(x_values, density_values, momentum_values, energy_values, N, dt, dx, gamma, simulation_time, scheme_name):
	...
	plt.title("Shu-Osher - {}: Density Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Density")
	...
	plt.title("Shu-Osher - {}: Velocity Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Velocity")
	...
	plt.title("Shu-Osher - {}: Pressure Variation (N = {})".format(scheme_name, N-1))
	plt.xlabel("x")
	plt.ylabel("Pressure")
...
saved_graphs_directory = "Shu_osher_graphs/"
